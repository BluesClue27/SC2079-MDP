    ...
    def snap_and_rec(self, obstacle_id_with_signal: str) -> None:
        ...
        if results['image_id'] == 'NA':
            self.failed_obstacles.append(
                self.obstacles[int(results['obstacle_id_with_signal'])])
            self.logger.info(
                f"Added Obstacle {results['obstacle_id_with_signal']} to failed obstacles.")
    ...
    def check_api(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.debug("Requesting from image API")
                url = f"http://{API_IP}:{API_PORT}/image"
                obstacle_id_with_signal = "Bulleseye_1"
